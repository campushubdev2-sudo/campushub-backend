...
class CalendarEntryController {
  ...
  createCalendarEntry = asyncHandler(async (req, res) => {
    const result = await calendarEntryService.createCalendarEntry(/** @type {AuthenticatedRequest} */ (req).user.id, req.body);
  ...
  getAll = asyncHandler(async (req, res) => {
    const result = await calendarEntryService.getCalendarEntries(/** @type {AuthenticatedRequest} */ (req).user?.id || null, req.query);
  ...
  getCalendarEntryById = asyncHandler(async (req, res) => {
    const result = await calendarEntryService.getCalendarEntryById(/** @type {AuthenticatedRequest} */ (req).user?.id || null, req.params.id);
  ...
  updateCalendarEntry = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const result = await calendarEntryService.updateCalendarEntry(/** @type {AuthenticatedRequest} */ (req).user.id, id, req.body);
  ...
  deleteCalendarEntry = asyncHandler(async (req, res) => {
    const result = await calendarEntryService.deleteCalendarEntry(/** @type {AuthenticatedRequest} */ (req).user.id, {
      id: req.params.id,
    });
  ...
  getCalendarStats = asyncHandler(async (req, res) => {
    const data = await calendarEntryService.getStats(/** @type {AuthenticatedRequest} */ (req).user.id);
