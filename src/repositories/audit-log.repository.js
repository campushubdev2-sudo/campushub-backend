...
class AuditLogRepository {
  /**
   * @param {{ userId?: string | import("mongoose").Types.ObjectId, action: string}} payload
  ...
  async findAll({ filter = {}, sort = "-createdAt", fields = "" }) {
    return AuditLog.find(filter).populate("userId", "username email role").sort(sort).select(fields).lean();
  }
